...
<?php 
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * from `playlist` where id = '{$_GET['id']}' ");
...
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		<div class="card-header">
			<h5 class="card-title"><?php echo isset($id) ? "Manage": "Create" ?> Playlist</h5>
		</div>
		<div class="card-body">
			<form id="playlist">
				<div class="row" class="details">
					...
					<div class="col-sm-6">
						<div class="form-group">
							<label for="title" class="control-label">Youtube Video_id</label>
				...
				<div class="row">
					<div class="col-sm-12">
						<div class="form-group">
							<label for="" class="control-label">Video Title (Optional)</label>
		...
		<div class="card-footer">
			<button class="btn btn-primary btn-sm" form="playlist"><?php echo isset($_GET['id']) ? "Update": "Save" ?></button>
			<a class="btn btn-primary btn-sm" href="./?page=playlist">Cancel</a>
		</div>
	</div>
</div>
<style>
...
function displayImg(input,_this) {
	...
	$(document).ready(function(){
		...
		$('#playlist').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Content.php?f=playlist",
				...
				success:function(resp){
					if(resp != undefined){
						if(resp.status == 'success'){
							location.href=_base_url_+"admin/?page=playlist";
						}else{
							alert_toast("An error occured",'error')
							console.log(resp);
						}
