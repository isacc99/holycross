...
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		<div class="card-header">
			<h5 class="card-title"><?php echo isset($id) ? "Manage": "Create" ?> Callery</h5>
		</div>
		<div class="card-body">
			<form id="gallery">
		...
		<div class="card-footer">
			<button class="btn btn-primary btn-sm" form="gallery"><?php echo isset($_GET['id']) ? "Update": "Save" ?></button>
			<a class="btn btn-primary btn-sm" href="./?page=gallery">Cancel</a>
		</div>
	</div>
</div>
<style>
...
function displayImg(input,_this) {
	...
	$(document).ready(function(){
		...
		$('#gallery').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Content.php?f=gallery",
				...
				success:function(resp){
					if(resp != undefined){
						if(resp.status == 'success'){
							location.href=_base_url_+"admin/?page=gallery";
						}else{
							alert_toast("An error occured",'error')
							console.log(resp);
						}
