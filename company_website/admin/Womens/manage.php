...
<?php 
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * from `womens` where id = '{$_GET['id']}' ");
...
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		...
		<div class="card-footer">
			<button class="btn btn-primary btn-sm" form="womens"><?php echo isset($_GET['id']) ? "Update": "Save" ?></button>
			<a class="btn btn-primary btn-sm" href="./?page=womens">Cancel</a>
		</div>
	</div>
</div>
<style>
...
function displayImg(input,_this) {
	...
	$(document).ready(function(){
		$('.select')
		$('#womens').submit(function(e){
			e.preventDefault();
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Content.php?f=womens",
				...
				success:function(resp){
					if(resp != undefined){
						if(resp.status == 'success'){
							location.href=_base_url_+"admin/?page=womens";
						}else{
							alert_toast("An error occured",'error')
							console.log(resp);
						}
