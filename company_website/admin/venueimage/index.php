		...
		<div class="card-body">
			<table class="table tabe-hover table-bordered table-compact" id="list">
				...
				<tbody>
					<?php
					$i = 1;
					$qry = $conn->query("SELECT * FROM `venueimage` order by unix_timestamp(`date_updated`) desc, unix_timestamp(`date_created`) desc");
...
<script>
	...
	$(document).ready(function(){
		$('.new_testimonial').click(function(){
			location.href = _base_url_+"admin/?page=venueimage/manage";
		})
		$('.manage_testimonial').click(function(){
			location.href = _base_url_+"admin/?page=venueimage/manage&id="+$(this).attr('data-id')
		})
		$('.delete_testimonial').click(function(){
		_conf("Are you sure to delete this image?","delete_testimonial",[$(this).attr('data-id')])
		})
		$('#list').dataTable()
	})
