...
$result = $conn->query("SELECT intro FROM dp WHERE title LIKE 'Men%' LIMIT 0, 25;");
$row = $result->fetch_assoc();
$mensDescription = $row['intro'];
?>
<?php
// Assuming $conn is the database connection variable

// Fetch data from the database
$query = $conn->query("SELECT * FROM mens ORDER BY date_created ASC LIMIT 1");
...
$query = $conn->query("SELECT * FROM leaders where fellowship = 'mens'");
$rows = $query->fetch_all(MYSQLI_ASSOC);
?>
...
$query_all = $conn->query("SELECT * FROM mens WHERE id <> {$oldest_row['id']}");
