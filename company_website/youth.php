...
$result = $conn->query("SELECT intro FROM dp WHERE title = 'Youth Fellowship'");
$row = $result->fetch_assoc();
$youthDescription = $row['intro'];
?>
<?php
// Assuming $conn is the database connection variable

// Fetch data from the database
$query = $conn->query("SELECT * FROM youth ORDER BY date_created ASC LIMIT 1");
...
$query = $conn->query("SELECT * FROM leaders where fellowship = 'youth'");
$rows = $query->fetch_all(MYSQLI_ASSOC);
?>
...
$query_all = $conn->query("SELECT * FROM youth WHERE id <> {$oldest_row['id']}");
